...
mi estimate, saving(condspkpartnerrecent_stage1, replace): logit condspkpartnerrecent ${demo_vars} ${sex_vars} ${source_vars} ${sup_vars}, cluster(ward_pt)
mi estimate, saving(condspkpartnerrecent_stage2, replace): logit condspkpartnerrecent ${demo_vars} ${sex_vars} ${source_vars} ${sup_vars} z_know z_beliefs z_control, cluster(ward_pt)
mi estimate, saving(condspkpartnerrecent_stage3, replace): logit condspkpartnerrecent ${demo_vars} ${sex_vars} ${source_vars} ${sup_vars} z_know z_beliefs z_control  fucond, cluster(ward_pt)
...
mi estimate, saving(condlast_stage4, replace): logit condlast ${demo_vars} ${sex_vars} ${source_vars} ${sup_vars} z_know z_beliefs z_control  fucond condspkpartnerrecent, cluster(ward_pt)
...
foreach cov in $demo_vars $sex_vars $source_vars $sup_vars z_know z_beliefs z_control fucond condspkpartnerrecent {
	...
	foreach outcome in fucond condspkpartnerrecent condlast {
		if "`cov'" != "`outcome'" {
			mi estimate, post: logit `outcome' `cov', cluster(ward_pt)
...
postclose `pf'
use "`tmp'", clear
save "../4_tables/t3_intentions.dta", replace

postclose `pf2'
use "`tmp2'", clear
save "../4_tables/t4_behaviors.dta", replace
